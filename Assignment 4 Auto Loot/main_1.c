//To compile: gcc -std=c11 -O2 main_1.c -o main_1.exe -lm
//To run: .\main_1.exe < sample_input.txt > sample_output.t
//.\main_1.exe < sample_input.txt > out_sample_input.txt
...
{item_find(r-1,c,rows-1,cols,array );}
else 
{item_taken++;
...
//printf("%d\n",bag_capacity);
//after there a while loop will read thourh each line of the file and store the rows in arrays
int total_item=0;
 //printf("Input:\n");
 while ( total_item < MAX_ITEMS) {
...
printf("%s, %d, %d\n",item_names[final_list[i]],item_features[final_list[i]][0],item_features[final_list[i]][1]);
//Print final weight and value 
printf("final weight: %d\n",bag_capacity);
printf("final value: %d\n",array[total_item][bag_capacity]);
